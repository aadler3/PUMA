...
# Hair variables
```{r}
puma_hair <- puma_all %>% select(ptid, tfvngml, tfvcomments, blq, tfvmodif, log2tfv, rx, hair3code, hair4code, hair4blq, hair5blq, hair3blq, detect_tfv)

puma_all <- puma_all %>% mutate(STRAND_hair = case_when(tfvngml < 0.012 ~ "Less than 2 doses per week",
                                                        (tfvngml >= 0.012) & (tfvngml <0.023) ~ "2 doses per week",
                                                        (tfvngml >= 0.023) & (tfvngml <0.038) ~ "4 doses per week",
                                                        tfvngml >= 0.038  ~ "7 doses per week"
                                                        ))

puma_all <- puma_all %>% mutate(STRAND_hair_bin = case_when(tfvngml < 0.023 ~ "<= 3",
                                                            tfvngml >= 0.023 ~ ">= 4"))

puma_all <- puma_all %>% relocate(STRAND_hair, STRAND_hair_bin, .after = tfvngml)
```
...
# Hair
hair_long <- puma_all %>% select(group, detect_tfv, STRAND_hair, STRAND_hair_bin, visit, ptid)

hair_long <- as.data.frame(hair_long)

hair_wide <- reshape(hair_long, v.names = c("detect_tfv", "STRAND_hair", "STRAND_hair_bin"), idvar = c("ptid", "group"), 
                   timevar = "visit", direction = "wide")
...
primary_long <- puma_all %>% select(ptid, group_new, visit, detect_tfv, ut_result, STRAND_hair,
                                    STRAND_hair_bin,
...
secondary_long <- puma_all %>% select(ptid, group_new, visit, detect_tfv, ut_result, STRAND_hair,
                                      STRAND_hair_bin,
