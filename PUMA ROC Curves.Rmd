...
hair2_results <- create_roc(month12$STRAND_hair_bin, month12$wilson_adh)

# Create legend labels with AUC values
urine_legend <- create_legend_with_auc("Urine (Neg vs Pos)", urine_results$auc)
hair_legend <- create_legend_with_auc("Hair (Neg vs Pos)", hair_results$auc)
hair2_legend <- create_legend_with_auc("Hair (Neg + Low vs High)", hair2_results$auc)
...
```{r}
#Calculate Prevalence
month12 <- month12 %>% mutate(STRAND_hair_bin_coded = ifelse(STRAND_hair_bin == ">= 4", 1, 0))

prevalence_hair2 <- sum(month12$STRAND_hair_bin_coded, na.rm = TRUE)/length(month12$STRAND_hair_bin_coded[!is.na(month12$STRAND_hair_bin_coded)])
...
predictors_df <- month12 %>% 
  select(ut_result, wilson_adh, STRAND_hair_bin_coded) %>% 
  filter(!is.na(STRAND_hair_bin_coded))

combined_results <- create_combined_roc(
  response = month12$STRAND_hair_bin_coded[!is.na(month12$STRAND_hair_bin_coded)],
...
plot1 <- create_roc_plot(hair2_results$roc, "A. Self-Report Adherence Prediction of Hair Results
     (0.0168 ng/mL as cutoff)", "purple", "Hair (0.023 ng/mL as cutoff), AUC")

# 2. Combined model (adherence score + urine 
plot2 <-  create_roc_plot(combined_results$roc, "C. Combined Self-Report Adherence and 
     Urine Result Prediction of Hair Results", "blue", "Hair (0.023 ng/mL as cutoff), AUC")

# 2. Urine outcome
plot3 <- create_roc_plot(urine_results$roc, "D. Self-Report Adherence Prediction of Urine Results", "red", "Urine (1500 ng/mL as cutoff), AUC")

# 3. Hair Outcome
plot4 <- create_roc_plot(hair_results$roc, "B. Self-Report Adherence Prediction of Hair Results 
     (0.002 ng/mL as cutoff)", "green", "Hair (0.002 ng/mL as cutoff), AUC")
