...
```{r}
# Visualization at Month 12
aggregated_data <- month12 %>% filter(!is.na(STRAND_hair)) %>%
  group_by(STRAND_hair) %>%
  summarise(
    mean_adh = mean(wilson_adh, na.rm = TRUE),
    sd_adh = sd(wilson_adh, na.rm = TRUE)
  )

aggregated_data <- aggregated_data %>% mutate(STRAND_hair = case_when(STRAND_hair == "2 doses per week" ~ "2-3",
                                                                      STRAND_hair == "4 doses per week" ~ "4-6",
                                                                      STRAND_hair == "7 doses per week" ~ "7",
                                                                      STRAND_hair == "Less than 2 doses per week" ~ "< 2"))

month12_plot <- ggplot(aggregated_data, aes(x = STRAND_hair, y = mean_adh)) +
...
month1_aggregate <- month1_data %>% filter(!is.na(STRAND_hair)) %>%
  group_by(STRAND_hair) %>%
  summarise(
    mean_adh = mean(wilson_adh, na.rm = TRUE),
    sd_adh = sd(wilson_adh, na.rm = TRUE)
...
month1_aggregate <- month1_aggregate %>% mutate(STRAND_hair = case_when(STRAND_hair == "2 doses per week" ~ "2-3",
                                                                      STRAND_hair == "4 doses per week" ~ "4-6",
                                                                      STRAND_hair == "7 doses per week" ~ "7",
                                                                      STRAND_hair == "Less than 2 doses per week" ~ "< 2"))

month1_plot <- ggplot(month1_aggregate, aes(x = STRAND_hair, y = mean_adh)) +
  geom_bar(stat = "identity", fill = "slategray2") +
  labs(
    title = "Baseline",
    x = "PrEP Doses per Week as Estimated by Hair TFV Levels",
...
month6_aggregate <- month6_data %>% filter(!is.na(STRAND_hair)) %>%
  group_by(STRAND_hair) %>%
  summarise(
    mean_adh = mean(wilson_adh, na.rm = TRUE),
    sd_adh = sd(wilson_adh, na.rm = TRUE)
...
month6_aggregate <- month6_aggregate %>% mutate(STRAND_hair = case_when(STRAND_hair == "2 doses per week" ~ "2-3",
                                                                      STRAND_hair == "4 doses per week" ~ "4-6",
                                                                      STRAND_hair == "7 doses per week" ~ "7",
                                                                      STRAND_hair == "Less than 2 doses per week" ~ "< 2"))

month6_plot <- ggplot(month6_aggregate, aes(x = STRAND_hair, y = mean_adh)) +
  geom_bar(stat = "identity", fill = "slategray2") +
  labs(
    title = "Month 6",
    x = "PrEP Doses per Week as Estimated by Hair TFV Levels",
