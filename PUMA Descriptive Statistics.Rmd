...
bin_variables <- c("marital", "sex8", "sex10", "sex12", "sex14", "par5___1", "par5___2",
                   ...
                   "par5___8", "par5___9",  "padh9___1", "padh9___2", 
...
table1_data <- table1_data %>% select("age", "edu", "income", "sex1", "prep_length", 
                                      "padh4_recode", "padh7_recode", "padh6", "emp_coded", 
...
table1_arms <- compareGroups(group~ ., data = table1_data, method = 2, na.omit(TRUE))
...
table1_all <- compareGroups(~ ., data = table1_data, method = 2, na.omit(TRUE))
...
hair_categorical_results <- analyze_categorical_across_visits(
  data = puma_all,
  var_names = c("STRAND_hair", "STRAND_hair_bin"),
  visits = 1:5,
  group_var = "group_new"
)
...
table(hair_wide$STRAND_hair_bin.1, hair_wide$group)
chisq.test(hair_wide$STRAND_hair_bin.1, hair_wide$group)

table(hair_wide$STRAND_hair_bin.2, hair_wide$group)
chisq.test(hair_wide$STRAND_hair_bin.2, hair_wide$group)

table(hair_wide$STRAND_hair_bin.3, hair_wide$group)
chisq.test(hair_wide$STRAND_hair_bin.3, hair_wide$group)

table(hair_wide$STRAND_hair_bin.4, hair_wide$group)
chisq.test(hair_wide$STRAND_hair_bin.4, hair_wide$group)

table(hair_wide$STRAND_hair_bin.5, hair_wide$group)
chisq.test(hair_wide$STRAND_hair_bin.5, hair_wide$group)
...
```{r}
## Hair
month12 <- month12 %>% mutate(strand_coded_bin = ifelse(STRAND_hair_bin == ">= 4", 1, 0))

hair_high <- month12 %>% filter(strand_coded_bin == 1)
hair_low <- month12 %>% filter(strand_coded_bin == 0)
